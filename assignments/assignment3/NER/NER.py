...
class NER(object):
    ...
    def mmap_read_word_vectors(self, fname):
    ...
    def capitalized_token(self):
        return self.current_token is not None and self.current_token.istitle()
    ...
    def __init__(self, training_file, test_file, model_file, word_vectors_file,
        ...
        self.Nn = 0  # Number of negative datapoints in training file.
        ...
        self.mo, self.fo = self.mmap_read_word_vectors(word_vectors_file)
        ...
        if training_file:
            ...
            if training_set:
                ...
                print("Negative datapoints: ", self.Nn)
                print("Total datapoints: ", len(training_set.x))
                b = BinaryLogisticRegression(training_set.x, training_set.y, self.Nn)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Named Entity Recognition',
                                     usage='\n* If the -d and -t are both given, the program will train a model, '
                                           'and apply it to the test file. \n* If only -t and -m are given, '
                                           'the program will read the model from the model file, and apply it to the '
                                           'test file.')
    ...
    ner = NER(args.d, args.t, args.m, args.w, args.s, args.mgd, args.lowercase_fallback)
    ner.mo.close()
    ner.fo.close()
