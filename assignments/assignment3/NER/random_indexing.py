...
class RandomIndexing(object):
    ...
    def create_word_vectors(self):
        def create_rv():
            rv = [0] * self.__dim
            values = random.choices(self.__non_zero_values, k=self.__non_zero)
        ...
        # Assign to each word its cv and rv
    ...
    def normalize_context_vectors(self):
        normalized_cv = {}
        for word, vector in self.__cv.items():
            norm = np.linalg.norm(vector)
            normalized_cv[word] = np.array(vector) / norm if norm != 0 else vector
        return normalized_cv

    #
    # @brief      Writes the resulting word vectors as a text file.
    #
    def write_word_vectors(self):
        normalized_cv = self.normalize_context_vectors()
        with open('RI_word_vectors.txt', 'w') as f:
            f.write('{} {}\n'.format(self.vocabulary_size, self.__dim))
            for word, vector in normalized_cv.items():
                f.write('{}'.format(word))
                for elem in vector:
                    f.write(' {}'.format(elem))
                f.write("\n")
    ...
    def train(self):
        ...
        self.write_word_vectors()
        spinner.succeed(text="Saved vectors to file! Execution is finished")
