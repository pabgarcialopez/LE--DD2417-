...
class NER(object):
    ...
    def token_contains_digit(self):
        return len(list(filter(lambda c: c in string.digits, self.current_token))) > 0

    def token_contains_punctuation(self):
        return len(list(filter(lambda c: c in string.punctuation, self.current_token))) > 0
        ...
        if training_file:
            ...
            if training_set:
                ...
                if stochastic_gradient_descent:
                    ...
                    b.fit()
                print("Elapsed time: ", time.time() - self.initTime)
